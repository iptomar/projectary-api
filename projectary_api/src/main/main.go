...
func main(){
	    ...
	    Authenticator: func(userId string, password string, c *gin.Context) (string, bool) {
				...
				err := db.Dbase.QueryRow("SELECT id,password FROM users WHERE username = ?", userId).Scan(&DBid,&DBpassword)
				if err != nil {
					if(DEBUG){
						fmt.Println("[DB-Debug] Error: ", err.Error())
					}
				}

				// Compare the stored password with the user given password
				err = bcrypt.CompareHashAndPassword([]byte(DBpassword), []byte(password))
				//Wrong password
				if err != nil {
					if (DEBUG){
						fmt.Println("[CRYPTO-Debug] err: ", err)
					}
					return userId, false
				}
