...
exports.nAssigned = function(req, res) {
  vendors.mysql.getConnection(function(err, connection) {
    connection.query('SELECT `group`.id as groupid, `group`.desc as groupname, project.id as projectid, project.name as projectname, project.description as projectdescription FROM `group` JOIN application ON application.groupid = `group`.id JOIN project ON project.id = application.projectid WHERE YEAR(application.approvedin) = 0000', function(error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': error });
      if (results < 1) {
        res.status(404).json({ 'result': 'nok', 'message': 'Application could not be found' });
