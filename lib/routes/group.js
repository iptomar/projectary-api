...
exports.join = function(req, res) {
  ...
  if (!params.password || !params.desc) return res.status(400).json({ 'result': 'nok', 'message': "Provide all data" });
  params.password = crypto.createHash('md5').update(params.password).digest('hex');
  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT * FROM `groupuser` WHERE `userid` = ?', id, function(error, results, fields) {
      ...
      if (results != 0) return res.status(403).json({ 'result': 'nok', 'message': "User already has a group" });

      // Check if the given group exists
      connection.query('SELECT * FROM `group` WHERE `desc` = ? LIMIT 1', params.desc, function(error, results1, fields) {
        if (error) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': error }); }
        if (results1 < 1) return res.status(404).json({ 'result': 'nok', 'message': "Group could not be found" });

        // Check if given password check with stored password
        else if (results1[0].password != params.password) { connection.release(); return res.status(400).json({ 'result': 'nok', 'message': "Invalid password" }); }
        var values = {
...
exports.remove = function(req, res) {
  ...
  if (req.user.isadmin != 1) return res.status(403).json({ 'result': 'nok', 'message': 'You don´t have permission to execute this action' });
  vendors.mysql.getConnection(function(err, connection) {
    // Delete the users from the group table
    connection.query('DELETE FROM `groupuser` WHERE groupid = ?', parseInt(req.params.id), function(error, results, fields) {
      if (error) return res.status(500).json({ 'result': 'nok', 'message': error });
      // Delete the group from table
      connection.query('DELETE FROM `group` WHERE id = ?', parseInt(req.params.id), function(error, results, fields) {
        connection.release();
        if (error) return res.status(500).json({ 'result': 'nok', 'message': error });
        res.status(200).json({ 'result': 'ok' });
      });
