...
exports.create = function(req, res) {
  var params = req.body;
  if (!params.password) return res.status(400).json({ 'result': 'nok', 'message': "Forneça uma palavra passe" });
    ...
    async.waterfall([
      ...
      function deleteGroup(callback) {
        bcrypt.genSalt(vendors.salt, function(err, salt) {
          bcrypt.hash(params.password, salt, function(err, hash) {
            connection.query('CALL insertNewGroup(?, ?, ?, @resultado)', [req.user.id, params.desc, hash], function(error, results, fields) {
              callback(error);
            });
          });
        });
      ...
      function checkResult(callback) {
        connection.query('SELECT @resultado as resultado', function(error, results, fields) {
          ...
          if (!results[0].resultado) callback("Ocorreu um erro ao criar o grupo");
...
exports.join = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT * FROM `groupuser` WHERE `userid` = ?', id, function(error, results, fields) {
      if (error) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error }); }
      if (results.length !== 0) return res.status(403).json({ 'result': 'nok', 'message': "O utilizador já está registado num grupo" });
      // Check if the given group exists
      connection.query('SELECT * FROM `group` WHERE `desc` = ? LIMIT 1', params.desc, function(error, results1, fields) {
        if (error) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error }); }
        if (results1.length < 1) return res.status(404).json({ 'result': 'nok', 'message': "Não foi possível encontrar o grupo" });
        // Check if given password match with stored password
        else {
          bcrypt.compare(params.password, results1[0].password, function(err, check) {
            if (check === false) { connection.release(); return res.status(400).json({ 'result': 'nok', 'message': "Password inválida" }); }
            else {
              var values = {
...
exports.list = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    connection.query('SELECT id, `desc` as name FROM `group`', function(error, results, fields) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
      if (results.length < 1) res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar grupos' });
  ...
  vendors.mysql.getConnection(function(err, connection) {
    async.waterfall([
      ...
      function(callback) {
        connection.query('SELECT @resultado as resultado', function(error, results, fields) {
          // If the result comes 'false'
          if (!results[0].resultado) callback("Ocorreu um erro ao eliminar o grupo");
          else callback(error);
        });
      }
    ], function(err, result) {
      ...
      connection.query('SELECT project.id, project.name FROM `group` JOIN application ON application.groupid = `group`.id JOIN project ON project.id = application.projectid WHERE YEAR(application.approvedin) != 0000 and `group`.id = ?', req.params.id, function(error, rest) {
        if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
          ...
          connection.query('SELECT user.id, user.name FROM user JOIN groupuser ON groupuser.userid = user.id WHERE groupuser.groupid = ?', parseInt(req.params.id), function(error, results1) {
            connection.release();
            if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
            if (results.length < 1) res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar o grupo' });
            else {
