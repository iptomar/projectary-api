...
exports.createStudent = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.beginTransaction(function(err) {
      if (err) return res.json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error' : err });
      // Insert the user
      connection.query('INSERT INTO user SET ?', values, function(error, results, fields) {
        if (error) return connection.rollback(function() { //if error rollback transaction
          ...
          res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error' : error });
        });
...
exports.swLock = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    async.waterfall([
      ...
      function getLockedState(callback) {
      ...
      function switchLock(error, results, callback) {
