...
exports.createStudent = function(req, res) {
  ...
  if (!params.password) return res.status(500).json({ 'result': 'nok', 'message': 'Password is required' });
  ...
  if (!values.name || !values.external_id || !values.email) return res.status(500).json({ 'result': 'nok', 'message': 'Provide all fields' });
  ...
  vendors.mysql.getConnection(function(err, connection) {
    connection.beginTransaction(function(err) {
      ...
      async.waterfall([
        function(callback) {
          bcrypt.genSalt(vendors.salt, function(err, salt) {
...
exports.list = function(req, res) {
  vendors.mysql.getConnection(function(err, connection) {
    connection.query('SELECT * FROM user', function(error, results) {
      connection.release();
      if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
      if (results.length < 1) res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar utilizadores' });
  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT * FROM USER WHERE id = ?', id, function(error, results) {
      ...
      connection.query('SELECT `group`.id AS groupid, `group`.desc AS groupname FROM `group`, groupuser, user WHERE groupuser.groupid = `group`.id AND groupuser.userid = user.id AND groupuser.userid = ?', id, function(error, results1, fields) {
        ...
        if (results.length < 1) res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar o utilizador' });
        else {
 ...
 * FIXME: Get a better way of doing this same query twice
 */
...
exports.chPassword = function(req, res) {
  ...
  if (!params.password) return res.status(400).json({ 'result': 'nok', 'message': 'Password Inválida' });

  if (req.user.isadmin === 1) {
    if (!params.id) return res.status(400).json({ 'result': 'nok', 'message': 'Id do Utilizador em falta' });
      ...
      bcrypt.genSalt(vendors.salt, function(err, salt) {
        bcrypt.hash(params.password, salt, function(err, hash) {
          connection.query('SELECT id FROM user WHERE id = ? LIMIT 1', parseInt(params.id), function(error, results, fields) {
            ...
            if (results[0].length < 1) return res.status(400).json({ 'result': 'nok', 'message': 'Utilizador não existe' });
            connection.query('UPDATE user SET password = ? WHERE id = ?', [hash, parseInt(params.id)], function(error, results) {
              connection.release();
              if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
              res.status(200).json({ 'result': 'ok' });
            });
    ...
    vendors.mysql.getConnection(function(err, connection) {
      ...
      connection.query('SELECT password FROM user WHERE id = ? LIMIT 1', req.user.id, function(error, results) {
        ...
        bcrypt.compare(params.password, results[0].password, function(err, check) {
          if (check === true) return res.status(400).json({ 'result': 'nok', 'message': 'Já utilizou esta password' });
          else {
            bcrypt.genSalt(vendors.salt, function(err, salt) {
              bcrypt.hash(params.password, salt, function(err, hash) {
                connection.query('UPDATE user SET password = ? WHERE id = ?', [hash, req.user.id], function(error, results) {
                  connection.release();
                  if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', 'error': error });
                  res.status(200).json({ 'result': 'ok' });
                });
      ...
      if (error || results.length != 1) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': 'Error fetching user (DB)' }); } else if (token && token.length > 1 && token === results[0].token) {
        ...
        bcrypt.genSalt(vendors.salt, function(err, salt) {
          bcrypt.hash(password, salt, function(err, hash) {
            connection.query('UPDATE user SET password = ? WHERE email = ?', [hash, results[0].email],
              function(error) {
                if (error) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': 'Error saving USER details (DB)' }); } else {
                  connection.release();
