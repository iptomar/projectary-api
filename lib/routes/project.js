  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT * FROM project WHERE ID = ?', parseInt(req.params.id), function(error, results, fields) {
      ...
      connection.query('SELECT `group`.id AS groupid FROM `group`, application WHERE application.groupid = `group`.id AND YEAR(application.approvedin) != 0000 AND application.projectid = ?', parseInt(req.params.id), function(error, results1, fields) {
        ...
        if (error) return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', error });
        if (results < 1) {
          res.status(404).json({ 'result': 'nok', 'message': 'Não foi posível encontrar o projecto' });
        } else {
...
/** TODO: Use async waterfall remove callback hell!!!!
...
exports.applInfo = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    ...
    connection.query('SELECT project.id, project.year, course.id as course, project.name, project.description, user.name as owner, project.created FROM project, course, user WHERE course.id = project.courseid and user.id = project.userid AND project.id = ? LIMIT 1', id, function(error, results) {
      if (error) {
        connection.release();
        return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', error });
      }
      if (results < 1) {
        connection.release();
        res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar candidaturas para o projeto especificado' });
      } else {
        ///Get info applications not approved
        connection.query('SELECT application.groupid FROM application WHERE application.projectid = ? and application.approvedin = "0000-00-00 00:00:00"', id, function(error, groups) {
          if (error) {
            ...
            if (groups < 1) { connection.release(); return res.status(404).json({ 'result': 'nok', 'message': 'Não foi possível encontrar candidaturas para o projeto especificado' }); } else async.mapSeries(groups, function(element, callb) {
              ///Get info user group
              ...
              if (err) { connection.release(); return res.status(500).json({ 'result': 'nok', 'message': 'Ocorreu um erro a executar a query.', err }); } else {
                async.mapSeries(groupinfo, function(element2, callbb) {
                  ...
                  connection.query('SELECT user.id, user.name FROM user, groupuser where groupuser.userid = user.id and groupuser.groupid = ?', element2.id, function(error, item2) {
                    if (error) callbb(error);
                      ...
                      //inject users + all info in the map series behind
  ...
  vendors.mysql.getConnection(function(err, connection) {
    async.waterfall([
      ...
      function deleteGroup(callback) {
        connection.query('CALL isFinished(?, ?, @resultado)', [req.params.userid, req.params.groupid], function(error, results, fields) {
          callback(error);
        });
      },
...
exports.attrGrade = function(req, res) {
  ...
  vendors.mysql.getConnection(function(err, connection) {
    async.waterfall([
      // Call procedure to delete the group
      function deleteGroup(callback) {
        connection.query('CALL insertGrade(?, ?, ?, ?, @resultado)', [req.params.userid, req.params.studentid, params.groupid, parseInt(params.grade) ], function(error, results, fields) {
          callback(error);
        });
      },
      // Check the query result
